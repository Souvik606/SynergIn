...
const ProfileHeader = ({ userData, onSave, isOwnProfile }) => {
  ...
  const [sendEditedData, setSendEditedData] = useState({});
  ...
  const isConnected = userData.connections.some((connection) => connection === authUser.data._id);
  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setSendEditedData((prev) => ({ ...prev, [event.target.name]: file }));
      };
    ...
    <div className='bg-white shadow rounded-lg mb-6'>
      ...
      <div className='p-4'>
          ...
          {isEditing ? (
            <input
              ...
              onChange={(e) => {
                ...
                setSendEditedData({ ...sendEditedData, name: e.target.value })
              }}
              className='text-2xl font-bold mb-2 text-center w-full rounded-full  py-2'
            />
          ) : (
          ...
          {isEditing ? (
            <input
              ...
              onChange={(e) => {
                ...
                setSendEditedData({ ...sendEditedData, headline: e.target.value })
              }}
              className='text-gray-600 text-lg text-center py-1 rounded-full w-full'
            />
          ) : (
            ...
            {isEditing ? (
              <input
                ...
                onChange={(e) => {
                  ...
                  setSendEditedData({ ...sendEditedData, location: e.target.value })
                }}
                className='text-lg text-gray-600 text-center py-1 rounded-full'
              />
            ) : (
        ...
        {isOwnProfile ? (
          isEditing ? (
            <button
              className='font-semibold w-full bg-primary text-white py-2 px-4 rounded-full hover:bg-primary-dark
